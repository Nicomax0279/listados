...
export const createRecord = async (date, initialPetsNumber) => {
  try {
    ...
    if (isNaN(initialPetsNumber))
      throw new Error("error  petsNumber is not a int number ");
...
export const putById = async (id, petsNumber) => {
  try {
    petsNumber = parseInt(petsNumber);
    if (isNaN(petsNumber))
      throw new Error("error  petsNumber is not a int number ");

      id = parseInt(id);
      if (isNaN(id)) throw new Error("error  id is not a int number ");
...
export const getByDate = async (date) => {
  try {
    ...
    if(!listing.estate)  throw new Error(`error listing with date:${date} is inactive and have id: ${listing.id}`);
    return listing;
  } catch (error) {
    throw error
  }
...
export const getById = async (id) => {
  try {
    ...
    const listing = await ListingModel.findByPk(id);
    if(!listing) throw new Error(`error listing with id:${id} not exist`);
    if(!listing.estate)  throw new Error(`error listing with id:${id} is inactive`);
    if(!listing.estate)  throw new Error(`error listing with date:${date} is inactive`);
    return listing;
  } catch (error) {
    throw error
  }
